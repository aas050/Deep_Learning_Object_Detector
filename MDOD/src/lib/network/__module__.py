...
class VarLevel5FPN(nn.Module):
    def __init__(self, input_chs, num_filters, upsampling, actv_func):
        ...
        self.stage_c6 = nn.Conv2d(input_chs[2], num_filters, 3, 2 * stride_fac, 1)
        self.stage_c7 = nn.Sequential(
            actv_func, nn.Conv2d(num_filters, num_filters, 3, 2 * stride_fac, 1))

        self.stage_p5_1 = nn.Conv2d(input_chs[2], num_filters, 1, 1 * stride_fac, 0)
        self.stage_p5_2 = nn.Conv2d(num_filters, num_filters, 3, 1 * stride_fac, 1)
        # self.stage_p5_up = nn.Upsample(scale_factor=2, mode=upsampling)

        self.stage_p4_1 = nn.Conv2d(input_chs[1], num_filters, 1, 1 * stride_fac, 0)
        self.stage_p4_2 = nn.Conv2d(num_filters, num_filters, 3, 1 * stride_fac, 1)
        # self.stage_p4_up = nn.Upsample(scale_factor=2, mode=upsampling)

        self.stage_p3_1 = nn.Conv2d(input_chs[0], num_filters, 1, 1 * stride_fac, 0)
        self.stage_p3_2 = nn.Conv2d(num_filters, num_filters, 3, 1 * stride_fac, 1)
...
class MyLevel5FPN(nn.Module):
    def __init__(self, input_chs, num_filters, upsampling):
        ...
        stride_fac = 1
        self.stage_c6 = nn.Conv2d(input_chs[3], num_filters, 3, 2 * stride_fac, 1)

        self.stage_p5_1 = nn.Conv2d(input_chs[3], num_filters, 1, 1 * stride_fac, 0)
        self.stage_p5_2 = nn.Conv2d(num_filters, num_filters, 3, 1, 1)
        self.stage_p5_up = nn.Upsample(scale_factor=2, mode=upsampling)

        self.stage_p4_1 = nn.Conv2d(input_chs[2], num_filters, 1, 1 * stride_fac, 0)
        self.stage_p4_2 = nn.Conv2d(num_filters, num_filters, 3, 1, 1)
        self.stage_p4_up = nn.Upsample(scale_factor=2, mode=upsampling)

        self.stage_p3_1 = nn.Conv2d(input_chs[1], num_filters, 1, 1 * stride_fac, 0)
        self.stage_p3_2 = nn.Conv2d(num_filters, num_filters, 3, 1, 1)
        self.stage_p3_up = nn.Upsample(scale_factor=2, mode=upsampling)

        self.stage_p2_1 = nn.Conv2d(input_chs[0], num_filters, 1, 1 * stride_fac, 0)
