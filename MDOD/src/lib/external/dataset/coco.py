...
class coco(imdb):
    def __init__(self, image_set, year, data_path):
        ...
        cats = self._COCO.loadCats(self._COCO.getCatIds(catNms=['cat', 'dog']))
        # Comment this for 80 classes
        cats = [{'supercategory': 'animal', 'id': 1, 'name': 'cat'}, {'supercategory': 'animal', 'id': 2, 'name': 'dog'}]
        self._classes = tuple(['__background__'] + [c['name'] for c in cats])
        self._class_to_ind = dict(list(zip(self.classes, list(range(self.num_classes)))))
        # Change this back for 80 classes
        self._class_to_coco_cat_id = dict(list(zip([c['name'] for c in cats], [1,2])))
    ...
    def _get_ann_file(self):
        ...
        print(osp.join(self._data_path, 'annotations', prefix + '_' + self._image_set + self._year + '.json'))
    ...
    def _do_detection_eval(self, res_file, output_dir):
        ...
        print(type(res_file))
        if type(res_file) == str:
            anns = json.load(open(res_file))
        elif type(res_file) == np.ndarray:
            anns = self._COCO.loadNumpyAnnotations(res_file)
        else:
            anns = res_file
    ...
    def _coco_results_one_category(self, boxes, cat_id):
        results = []
        for im_ind, index in enumerate(self._COCO.getImgIds(catIds=[17, 18])):
